...
def get_reports_dir(settings: Settings, job_id: str) -> Path:
    return get_job_dir(settings, job_id) / 'reports'
...
async def save_upload_file(*, upload: UploadFile, destination: Path, max_size_bytes: int) -> int:
        ...
        while chunk := await upload.read(1024 * 1024):
            ...
            if total_size > max_size_bytes:
                ...
                raise AppError(
                    ...
                    message='El fichero supera el tamano maximo permitido.',
                    status_code=status.HTTP_413_REQUEST_ENTITY_TOO_LARGE,
                    details={'maxUploadMb': max_size_bytes // (1024 * 1024)},
                )
            output.write(chunk)
