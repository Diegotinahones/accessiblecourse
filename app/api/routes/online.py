...
def get_canvas_auth(
    canvas_base_url: str = Header(..., alias="X-Canvas-Base-Url"),
    canvas_token: str = Header(..., alias="X-Canvas-Token"),
) -> CanvasCredentials:
    try:
        return CanvasCredentials.create(base_url=canvas_base_url, token=canvas_token)
    except AppError:
        raise
...
def create_online_job(
    ...
    request.app.state.online_job_contexts.put(
        response.jobId,
        OnlineJobContext(credentials=auth, course_id=course.id, course_name=course.name),
    )
